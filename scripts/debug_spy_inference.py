"""
Debug script: run one spy game and save all 16 generated images.
4 games × 4 players = 16 images, each via SDE sampling.
"""
import os
import torch
...
model = load_checkpoint_and_dispatch(
    ...
    device_map={"": device},
    offload_buffers=False, dtype=dtype, force_hooks=True,
...
# Minimal grpo_config needed by inferencer
...
# ── Generate 4 games × 4 players ──
game_gen = SpyGameDataGenerator(num_players=4)
G = 4
N = 4

print(f"\nGenerating {G} games × {N} players = {G*N} images...")

for g in range(G):
    game_data = game_gen.generate_game(epoch=0, sample_idx=g)
    spy = game_data['spy_player']
    print(f"\n=== Game {g+1} (spy=Player {spy}) ===")
    print(f"  Original: {game_data['original_description'][:80]}...")
    print(f"  Modified: {game_data['modified_description'][:80]}...")

    for pid in range(1, N + 1):
        prompt = game_gen.format_generation_prompt_simple(game_data, pid)
        role = "SPY" if pid == spy else "CIV"
        print(f"  Generating Player {pid} ({role})...", end=" ", flush=True)
        ...
        img_tensor = output['image']  # [3, H, W]
        img_np = (img_tensor * 255).round().clamp(0, 255).to(torch.uint8).cpu().numpy()
        img_np = img_np.transpose(1, 2, 0)  # CHW → HWC
        pil = Image.fromarray(img_np)

        fname = f"game{g+1}_player{pid}_{role}.jpg"
        pil.save(os.path.join(OUTPUT_DIR, fname))
        print(f"saved → {fname}")

print(f"\nAll {G*N} images saved to {OUTPUT_DIR}/")
