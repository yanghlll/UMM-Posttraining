...
def run_bagel_vote_multi_image_repeated(inferencer, player_images: List[Image.Image],
    ...
    Input format (interleaved, matching Bagel's official multi-image pattern):
      "Player 1's generated image:" [img1]
      "Player 2's generated image:" [img2]
      ...
      <vote_prompt>

    Each player label is placed immediately before its image in the KV cache,
    so the model can clearly associate each image with its label.
    ViT encoding is done once; K votes are generated by deepcopy of KV cache.
    """
    ...
    with torch.no_grad(), torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        ...
        # Interleaved: [label_text_i] [image_i] for each player, then vote_prompt
        for i, image in enumerate(images_rgb):
            # Text label before image
        ...
        # Final vote prompt after all player images
        generation_input, newlens, new_rope = model.prepare_prompts(
            curr_kvlens=newlens, curr_rope=new_rope,
            prompts=["\n" + vote_prompt], tokenizer=tokenizer,
            new_token_ids=new_token_ids,
        )
        for k, v in generation_input.items():
            if torch.is_tensor(v):
        ...
        # Generate K votes: deepcopy KV cache + generate_text
        for _ in range(num_generations):
            ...
            del kv_copy  # Free memory immediately
...
def sample_vote_with_logprobs(inferencer, player_images: List[Image.Image],
    ...
    results = []
    with torch.no_grad(), torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        # Build KV cache: images + text prompt
        past_key_values = NaiveCache(model.config.llm_config.num_hidden_layers)
        newlens = [0]
        new_rope = [0]
        ...
        # Generate K votes with log_probs
        for _ in range(num_generations):
            kv_copy = deepcopy(past_key_values)
            gen_input = model.prepare_start_tokens(newlens, new_rope, new_token_ids)
            for k, v in gen_input.items():
            ...
            # gen_ids: [steps+1, 1], gen_logps: [steps, 1] (batch=1)
            token_ids = gen_ids[1:, 0]  # skip start token
            log_probs = gen_logps[:, 0]

            text = tokenizer.decode(gen_ids[:, 0])
            text = text.split('<|im_end|>')[0]
            if '<|im_start|>' in text:
                text = text.split('<|im_start|>')[1]
...
def compute_decision_rewards(vote_samples, spy_player, num_players, extract_vote_fn,
    ...
    for vs in vote_samples:
        ...
        # Format check: has <answer> or \boxed{} with a valid number
        fmt_ok = extracted is not None and extracted.get("voted_spy") is not None
...
def compute_text_grpo_loss(model, tokenizer, new_token_ids, inferencer,
                            ...
                            advantage, clip_range=0.2,
                            max_tokens=512, temperature=0.7):
    """Compute text GRPO loss for one vote sample (Vision-Zero decision phase).

    Per-token PPO-clip loss: -min(r*A, clip(r,1-e,1+e)*A)
    where r = exp(log_pi_new - log_pi_old), A = advantage
    ...
    # Rebuild KV cache (no grad for prompt encoding)
    with torch.no_grad():
        past_key_values = NaiveCache(model.config.llm_config.num_hidden_layers)
        newlens = [0]
        new_rope = [0]
    ...
    # Forward pass WITH gradients: feed target tokens, get logits
    extra_inputs = {"mode": "und"} if model.use_moe else {}
    batch_size = 1

    # Build full input: [bos_token, target_ids[0], target_ids[1], ...]
    bos = new_token_ids['bos_token_id']
    if torch.is_tensor(bos):
        input_tokens = torch.cat([bos.unsqueeze(0).to(device), target_ids])
    else:
        input_tokens = torch.cat([torch.tensor([bos], device=device), target_ids])

    new_log_probs = []
    kv = past_key_values
    kv_lens = torch.tensor(newlens, dtype=torch.int, device=device)
    rope_pos = torch.tensor(new_rope, dtype=torch.long, device=device)

    # Teacher-force: feed each token, get log_prob of next token
    for t in range(seq_len):
        curr_token = input_tokens[t:t+1]
        packed_text_embedding = model.language_model.model.embed_tokens(curr_token)
        query_lens = torch.ones(1, dtype=torch.int, device=device)
        packed_query_indexes = torch.tensor([kv_lens.sum().item()], device=device)

        pki = torch.arange(kv_lens.sum().item(), device=device)

        output = model.language_model.forward(
            packed_query_sequence=packed_text_embedding,
            query_lens=query_lens,
            packed_query_position_ids=rope_pos,
        ...
        token_logp = log_p[0, target_ids[t].item()]
        new_log_probs.append(token_logp)
        kv_lens = kv_lens + 1
        rope_pos = rope_pos + 1

    new_log_probs = torch.stack(new_log_probs)  # [seq_len]
