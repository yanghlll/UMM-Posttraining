...
class SpyGameDataGenerator:
    ...
    def format_generation_prompt(self, game_data, player_id, num_prev_images=0):
        ...
        Simply gives the scene description — no game rules, no role info.
        CIV gets original description, SPY gets modified description.
        The difference in generated images comes purely from the prompt difference.
        """
        desc = game_data["player_descriptions"][player_id - 1]
        return f"Generate a high-quality image: {desc}"
...
class TextFileGameDataGenerator(SpyGameDataGenerator):
        ...
        if prompt_type == 'ocr':
            ...
            with open(jsonl_path, 'r') as f:
                for line in f:
                    ...
                    # Skip two_object: swap A↔B produces near-identical images
                    if data.get('tag') == 'two_object':
                        continue
    ...
    def format_generation_prompt(self, game_data, player_id, num_prev_images=0):
        """Generation prompt — just the description, no game rules."""
        desc = game_data["player_descriptions"][player_id - 1]
        return f"Generate a high-quality image: {desc}"
