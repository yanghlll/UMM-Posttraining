...
class SpyGameDataGenerator:
    ...
    def format_generation_prompt(self, game_data, player_id, num_prev_images=0):
        ...
        Args:
            ...
            num_prev_images: Number of previous players' images visible to this player.
        """
        ...
        if player_id == spy_player:
            role_hint = (
                ...
                f"from the other players'. "
            )
            prev_context = (
                f"You can see the images generated by the previous {num_prev_images} "
                f"player(s) above. Use them as reference to make your image "
                f"to blend in (if you are the spy). But you cannot generate an identical image, "
                f"if the generated image is the same as the before, you will be more severely punished...\n\n"
            )
            generate_instruct = (
                f"Please refer to the scene description below, which differs slightly from the other players', and any previously seen images (if you are not Player 1), to generate a high-quality image. "
            )
        else:
            role_hint = (
                f"You are PLAYER {player_id} of {N} in a game of \"Who's the Odd One Out?\". "
                f"Your role: CIVILIAN. "
            )
            prev_context = ""
            generate_instruct = (
                f"Generate a high-quality image of the following scene. "
            )
        
        game_instruction = (
            f"Game Instructions:\n"
            f"• There are 4 players...\n"
            f"• Player who are voted as the odd player will be severely punished...\n\n"
            f"What you should do:\n"
            f"If NOT spy: Generate an image that faithfully represents the scene description.\n"
            f"If spy: Be strategic Generate an image that looks as similar as possible to what the original scene might look like, "
            f"to avoid detection. But it must still remain consistent with your own scene description, otherwise you will be more severely punished..."
        )


        if num_prev_images > 0:
            prev_context = (
                f"You can see the images generated by the previous {num_prev_images} "
                f"player(s) above. Use them as reference to make your image consistent "
                f"(if you are a civilian) or to blend in (if you are the spy). But you cannot generate an identical image, "
                f"if the generated image is the same as the before, you will be more severely punished...\n\n"
            )
        else:
            prev_context = ""
    ...
    def build_generation_input_list(self, game_data, player_id, prev_images):
        """Build interleaved input list for generation with previous images visible.

        Returns a list like:
          ["Player 1's image:", PIL_img1, "Player 2's image:", PIL_img2, ..., gen_prompt]
        suitable for inferencer.interleave_inference().

        Args:
            game_data: Game data dict.
            player_id: 1-indexed current player ID.
            prev_images: List of PIL images from previous players (can be empty).
        """
        input_list = []
        for i, img in enumerate(prev_images):
            input_list.append(f"Player {i+1}'s generated image:")
            input_list.append(img)

        prompt = self.format_generation_prompt(game_data, player_id,
                                                num_prev_images=len(prev_images))
    ...
    def extract_vote(self, response):
        ...
        Priority:
          1. \\boxed{N} format
          2. <answer>N</answer> format (with or without closing tag)
          3. Return None (no fallback full-text search to avoid false matches)
        """
        ...
        # 1. Try \\boxed{} format first (Vision-Zero primary format)
        m = re.search(r'\\\\?boxed\{(.*?)\}', response, re.DOTALL)
        if m:
            answer_content = m.group(1).strip()

        # 2. Try <answer>...</answer> format
        if answer_content is None:
            m = re.search(r'<answer>(.*?)</answer>', response, re.DOTALL)
            if m:
                answer_content = m.group(1).strip()

        # 3. Try <answer> without closing tag (extract everything after it)
        if answer_content is None:
    ...
    def calculate_game_rewards(self, game_data, player_votes):
        ...
        spy_caught = (vote_counts[spy] == max_v and
                      sum(1 for v in vote_counts.values() if v == max_v) == 1)
        return {
    ...
    def format_generation_prompt(self, game_data, player_id, num_prev_images=0):
        ...
        if num_prev_images > 0:
            prev_context = (
                f"You can see the images generated by the previous {num_prev_images} "
                f"player(s) above. Use them as reference to make your image consistent "
                f"(if you are a civilian) or to blend in (if you are the spy).\n\n"
            )
        else:
            prev_context = ""
